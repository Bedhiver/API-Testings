...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your Fullapirest account.<br>
        ...
        &mdash; Fullapirest Team
      `
      return sendMail({ toEmail: email, subject: 'Fullapirest - Password Reset', content })
    })
